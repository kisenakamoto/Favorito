...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM furnitures";
	$run_query = mysqli_query($con,$category_query) or die(mysqli_error($con));
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h1>Furnitures</h1></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$cid = $row["furnitures_id"];
			$cat_name = $row["furnitures_title"];
			echo "
					<h3><li><a href='#' class='category' cid='$cid'>$cat_name</a></li></h3>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["brand"])){
	$brand_query = "SELECT * FROM appliances";
	$run_query = mysqli_query($con,$brand_query);
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active'><a href='#'><h1>Appliances</h1></a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$bid = $row["appliances_id"];
			$brand_name = $row["appliances_title"];
			echo "
					<h3><li><a href='#' class='selectBrand' bid='$bid'>$brand_name</a></li></h3>
			";
		}
		echo "</div>";
	}
							...
							echo   
								'<input type="hidden" name="return" value="http://localhost/project1/payment_success.php"/>
									...
									<input style="float:right;margin-right:80px;" type="image" name="submit"
										src="images/paypal.png" alt="PayPal Checkout"
